...
#define one_arg_reduce_fn_body(name, _stride, itype, vec, otype, init_fn, vreduce_fn, hreduce_fn) \
  otype BMAS_##name(const long n, itype* x, const long incx){           \
    ...
    if (incx == 1){                                                     \
      ...
      while(x != simd_end){                                             \
        v = vec##_load(x);                                              \
        acc = BMAS_vector_##vreduce_fn(acc, v);                         \
        x += stride;                                                    \
      }                                                                 \
    }else{                                                              \
      long i=0;                                                         \
    ...
    otype result = BMAS_vector_##hreduce_fn(acc);                       \
    while(x!=x_end){                                                    \
      result = BMAS_scalar_##vreduce_fn(result, x[0]);                  \
      x += incx;                                                        \
    }                                                                   \
    return result;                                                      \
  }
